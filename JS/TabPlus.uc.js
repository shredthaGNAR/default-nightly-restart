...
(async function (css) {
    ...
    TabPlus.modules.verticalTabPaneSwitchOnHover = {
        ...
        handleEvent(event) {
            ...
            if (!tab) return;
            var timeout = setTimeout(() => tab.click(), cPref.get('browser.tabs.switchOnHoverDelay', cPref.get('browser.tabs.switchOnHoverDelay', 2000)));
        },
        destroy(win) {
            win || (win = window);
            let list = win.document.getElementById("vertical-tabs-list");
    ...
    function $L(str, replace) {
        const LOCALE = LANG[Services.locale.defaultLocale] ? Services.locale.defaultLocale : 'en-US';
        if (str) {
